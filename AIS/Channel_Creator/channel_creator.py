...
class Abstract_Channel_Creator:
    ...
    def __init__(self, sparc4_operation_mode):
    ...
    def apply_sparc4_spectral_response(self, specific_flux, l_init, l_final, l_step):
        ...
        self._S4_SR.write_specific_flux(specific_flux, wavelength_interval)
        if self.sparc4_operation_mode == "pol":
            self._S4_SR.apply_polarimetric_component_spectral_response(
                "calibration_wheel"
            )
            self._S4_SR.apply_polarimetric_component_spectral_response("retarder")
            self._S4_SR.apply_polarimetric_component_spectral_response("analyser")
        self._S4_SR.collimator()
        self._S4_SR.apply_photometric_component_spectral_response("dichroic")
        self._S4_SR.apply_photometric_component_spectral_response("camera")
        self._S4_SR.apply_photometric_component_spectral_response("ccd")
        specific_flux = self._S4_SR.get_specific_flux()
