...
csv_file_name = os.path.join(BASE_PATH, "polarizer_contrast_ratio.csv")
ss = pd.read_csv(csv_file_name)
sys_wavelength, contrast_ratio = np.asarray(ss["Wavelength (nm)"]), np.asarray(
    ss["Contrast ratio"]
)
...
def channel():
    ch = Channel(CHANNEL)
    ch.obj_wavelength = obj_wavelength
    return ch
...
depolarizer_path = os.path.join(BASE_PATH, "depolarizer.csv")
...
collimator_path = os.path.join(BASE_PATH, _PHOT_OPTICAL_COMPONENTS["collimator"])
...
def test_get_polarizer_contrast_ratio(channel):
    ...
    ss = pd.read_csv(csv_file_name)
    sys_wavelength, new_contrast_ratio = np.asarray(ss["Wavelength (nm)"]), np.asarray(
        ss["Contrast ratio"]
    )
    spl = interp1d(
        sys_wavelength,
